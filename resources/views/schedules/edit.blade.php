<x-app-layout>
    ...
    @section('breadcrumb', 'Update an existing class schedule')
    ...
    @if (session('error'))
        <div
            class="flex items-center gap-3 bg-red-50 border border-red-200 text-red-800 text-sm rounded-xl px-4 py-3 mb-6">
            {{ session('error') }}
        </div>
    @endif
    ...
    <div class="bg-white rounded-xl border border-slate-200 max-w-3xl">
        ...
        <form id="edit-schedule-form" action="{{ route('schedules.update', $schedule) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="px-6 py-5 border-b border-slate-100">
                <div class="flex items-start justify-between">
                    <div>
                        <h2 class="text-sm font-semibold text-slate-800 font-sans">Edit Schedule</h2>
                        <p class="text-xs text-slate-400 mt-0.5">Update the details for schedule record
                            #{{ $schedule->id }}</p>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <button form="edit-schedule-form" type="submit"
                            ...
                            <select id="school_year_id" name="school_year_id"
                                ...
                                @foreach ($schoolYears as $schoolYear)
                                    <option value="{{ $schoolYear->id }}"
                                        {{ old('school_year_id', $schedule->school_year_id) == $schoolYear->id ? 'selected' : '' }}>
                                        {{ $schoolYear->name }}
                                    </option>
                                @endforeach
                            </select>
