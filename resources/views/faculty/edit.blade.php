<x-app-layout>
    ...
    @section('breadcrumb', 'Update faculty record')
        ...
        <form action="{{ route('faculty.update', $faculty) }}" method="POST">
            ...
            <div class="px-6 py-5 border-b border-slate-100">
                <div class="flex items-start justify-between">
                    <div>
                        <h2 class="text-sm font-semibold text-slate-800 font-sans">Edit Faculty</h2>
                        <p class="text-xs text-slate-400 mt-0.5">Update the details for {{ $faculty->full_name }}</p>
                    </div>
            ...
            <div class="p-6 space-y-12">
                ...
                {{-- Section: Portal Account (read-only) --}}
                <div>
                    <div class="py-3 border-b border-slate-100 mb-4">
                        <h3 class="text-xs font-semibold text-slate-500 uppercase tracking-widest font-sans">Portal
                            Account</h3>
                        <p class="text-xs text-slate-400 mt-0.5">Employee number and email cannot be changed here</p>
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-2">Employee Number</label>
                            <input type="text" value="{{ $faculty->employee_number }}" disabled
                                class="w-full px-4 py-2 text-sm border border-slate-200 rounded-lg text-slate-400 bg-slate-50 cursor-not-allowed">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-2">Email Address</label>
                            <input type="text" value="{{ $faculty->user?->email ?? '—' }}" disabled
                                class="w-full px-4 py-2 text-sm border border-slate-200 rounded-lg text-slate-400 bg-slate-50 cursor-not-allowed">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <input type="text" id="first_name" name="first_name"
                                value="{{ old('first_name', $faculty->first_name) }}" placeholder="Juan"
                        ...
                        <div>
                            ...
                            <input type="text" id="middle_name" name="middle_name"
                                value="{{ old('middle_name', $faculty->middle_name) }}" placeholder="Santos"
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="last_name" name="last_name"
                            value="{{ old('last_name', $faculty->last_name) }}" placeholder="Dela Cruz"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <input type="date" id="birth_date" name="birth_date"
                                value="{{ old('birth_date', $faculty->birth_date?->format('Y-m-d')) }}"
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="contact_number" name="contact_number"
                            value="{{ old('contact_number', $faculty->contact_number) }}" placeholder="09XX XXX XXXX"
                    ...
                    <div>
                        ...
                        <textarea id="address" name="address" rows="3" placeholder="Street, Barangay, City, Province"
                            class="w-full px-4 py-2 text-sm border border-slate-300 rounded-lg text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-school-600 focus:border-school-600 @error('address') border-red-500 @enderror">{{ old('address', $faculty->address) }}</textarea>
                        @error('address')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <input type="text" id="department" name="department"
                                value="{{ old('department', $faculty->department) }}" placeholder="e.g. Mathematics"
                        ...
                        <div>
                            ...
                            <input type="text" id="position" name="position"
                                value="{{ old('position', $faculty->position) }}" placeholder="e.g. Department Head"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="rank" name="rank"
                                ...
                                <option value="instructor" {{ old('rank', $faculty->rank) === 'instructor' ? 'selected' : '' }}>Instructor</option>
                                <option value="assistant_professor" {{ old('rank', $faculty->rank) === 'assistant_professor' ? 'selected' : '' }}>Assistant Professor
                                </option>
                                <option value="associate_professor" {{ old('rank', $faculty->rank) === 'associate_professor' ? 'selected' : '' }}>Associate Professor
                                </option>
                                <option value="professor" {{ old('rank', $faculty->rank) === 'professor' ? 'selected' : '' }}>Professor</option>
                            </select>
                            @error('rank')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                ...
                <div>
                    <div class="py-3 border-b border-slate-100 mb-4">
                        <h3 class="text-xs font-semibold text-slate-500 uppercase tracking-widest font-sans">Employment
                        </h3>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="employment_type" name="employment_type"
                                ...
                                <option value="full_time" {{ old('employment_type', $faculty->employment_type) === 'full_time' ? 'selected' : '' }}>Full-time</option>
                                <option value="part_time" {{ old('employment_type', $faculty->employment_type) === 'part_time' ? 'selected' : '' }}>Part-time</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select id="status" name="status"
                                ...
                                <option value="retired" {{ old('status', $faculty->status) === 'retired' ? 'selected' : '' }}>Retired</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
