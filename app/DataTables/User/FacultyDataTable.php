...
class FacultyDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->filterColumn(
                'employee_number',
                fn($query, $keyword) =>
                $query->where('employee_number', 'ilike', "%{$keyword}%")
            )
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('department')->title('Department')->searchable(true),
            Column::make('status')->title('Status')->searchable(false)->orderable(false),
            Column::make('created_at')->title('Date Added')->searchable(true),
